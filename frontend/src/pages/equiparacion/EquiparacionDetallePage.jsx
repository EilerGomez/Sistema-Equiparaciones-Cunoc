...
export default function EquiparacionDetallePage(){
  ...
  async function pdf(download=false){
    setPdfBusy(true)
    try{
      const {data}=await api.get(`/equiparaciones/${id}/pdf`,{params:{download:download?'1':'0'},responseType:'blob',timeout:60000})
      const url=URL.createObjectURL(data)
      if(download){
        const link=document.createElement('a')
        link.href=url
        link.download=`equiparacion-${detail.codigo}.pdf`
