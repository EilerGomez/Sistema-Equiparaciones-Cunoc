...
export default function ImportarEquiparacion({onClose,onCreated}){
  ...
  return <div className="eq-overlay"><section className="eq-dialog eq-import-dialog" role="dialog" aria-modal="true" aria-labelledby="import-title">
    ...
    {preview&&selection&&<form onSubmit={save}>
      <div className="eq-form-section"><h3>01 / Documento extraido</h3><div className="eq-import-summary"><div><small>Dictamen de origen</small><strong>{extracted.codigo}</strong><span>Solo para trazabilidad · {extracted.subfijo}</span></div><div><small>Fecha del PDF</small><strong>{extracted.fecha_impresion}</strong><span>{extracted.sede}</span></div><div><small>Nueva equiparacion</small><strong>Numero propio al guardar</strong><span>Estado PENDIENTE · {extracted.cursos.length} cursos</span></div></div>
      ...
      <div className="eq-form-section"><h3>03 / Pensums y autoridades</h3><div className="eq-form-grid halves">{[['id_pensum_de','Origen',extracted.origen],['id_pensum_a','Destino',extracted.destino]].map(([key,label,source])=><label key={key}>{label} · PDF: {source.carrera} {source.anio} {source.institucion}<select required value={selection[key]} onChange={event=>setSelection(current=>({...current,[key]:event.target.value}))}><option value="">Seleccionar pensum</option>{catalogs.pensums.filter(p=>key==='id_pensum_de'||Number(p.vigencia)===1).map(p=><option key={p.id} value={p.id}>{p.carrera} · {p.anio} · {p.institucion}{Number(p.vigencia)?' · Vigente':''}</option>)}</select></label>)}</div>
        <p className="eq-hint">Coordinador de destino: {coordinador?.nombre||'Falta configurar'} · Director: {catalogs.director?.nombre||'Falta configurar'}</p>
      </div>
      <div className="eq-form-section"><h3>04 / Cursos leidos del PDF</h3><div className="eq-course-list eq-import-courses"><table className="eq-table"><thead><tr><th>No.</th><th>Origen</th><th>Destino</th><th>% de BD</th><th>Opinion de BD</th></tr></thead><tbody>{extracted.cursos.map(row=><tr key={row.numero}><td>{row.numero}</td><td><small>{row.curso_de_codigo}</small>{row.curso_de_nombre}</td><td><small>{row.curso_a_codigo}</small>{row.curso_a_nombre}</td><td>{row.porcentaje}%{!row.equivalencia_existente&&<small>Valor inicial</small>}</td><td>{row.opinion}{!row.equivalencia_existente&&<small>Par nuevo</small>}</td></tr>)}</tbody></table></div><p className="eq-hint">El porcentaje y la opinion se toman de las equivalencias guardadas en la base de datos. Si el par no existe, se creara con 100% y EQUIVALENTE. Los ceros iniciales de los codigos se omiten.</p></div>
      <div className="eq-form-footer"><span>Se asignara el siguiente numero anual de equiparacion. {extracted.codigo} quedara como dictamen de origen.</span><button type="button" className="eq-btn" onClick={onClose} disabled={busy}>Cancelar</button><button type="submit" className="eq-btn primary" disabled={busy||!selection.id_sede||!selection.id_pensum_de||!selection.id_pensum_a||!coordinador||!catalogs.director}>{busy?'Guardando...':'Confirmar e importar'}</button></div>
    </form>}
  </section></div>
}
